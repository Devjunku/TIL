...
def move_shark(x, y, cnt, del_fish, direction, check):
    ...
    # TODO 상어의 방향을 굳이 만들지 않고, 있는거에서 index만 추출해서 씀
    for d in [2, 0, 4, 6]:
        nx, ny = x + dx[d], y + dy[d]
        ...
        if [nx, ny] in check: flag = 1
        if flag == 0: del_fish += len(fish[nx][ny])
        if fish[nx][ny]: check.append([nx, ny])
